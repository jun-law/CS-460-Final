...
filter_required() is a helper method I added to filter out edges that are not located between two required nodes or a required node and the exit node, to be used in precompute_distances(). Previously, I had created a dictionary named result in precompute_distances and tried assigning the result of calling run_dijkstra(), which returns a dictionary, as an entry. This created a nested dictionary and it did not allow me to access the items in the result as I wished. I was able to fix this issue by simply assigning the result of calling run_dijkstra() to a variable instead.
